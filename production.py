from app_types import Question, QuestionType
from utils import *
import re
    ...
    for q in data:
        ...
        if q.type == QuestionType.YES_NO:
            ...
            while True:
                ...
                # values from dict, index based
                values = []

                # pretty print all the variants
                print(q.question, end=' ')
                for index, (key, value) in enumerate(q.response.items()):
                    print(f'{key} ({index}) ', end='')
                    values.append(value)
